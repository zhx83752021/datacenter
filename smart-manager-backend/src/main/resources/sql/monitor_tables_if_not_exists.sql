-- 首次部署缺表时由 EnsureMonitorTablesRunner 执行；不 DROP，可重复跑。无 USE，使用 DataSource 已选中的库。

CREATE TABLE IF NOT EXISTS sm_indicator_category (
...
CREATE TABLE IF NOT EXISTS sm_indicator_lib (
  id BIGINT NOT NULL AUTO_INCREMENT COMMENT '主键ID',
...
CREATE TABLE IF NOT EXISTS sm_indicator_value (
  id BIGINT NOT NULL AUTO_INCREMENT COMMENT '主键ID',
...
CREATE TABLE IF NOT EXISTS sm_target_config (
  id BIGINT NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  indicator_id BIGINT NOT NULL COMMENT '指标ID',
